...
class Pool extends EventEmitter {
  ...
  /**
   * 
   * @param pooledResource
   * @param option {{[ensureMinimum]:Boolean}}
   * @private
   */
  _destroy(pooledResource,option) {
    if (!option) option = {};
    // FIXME: do we need another state for "in destruction"?
    pooledResource.invalidate(); //标记为无效
  ...
  _testOnBorrow() {
    ...
    pooledResource.test(); //标记为测试中
    this._testOnBorrowResources.add(pooledResource);
    const validationPromise = this._factory.validate(pooledResource.obj);
    const wrappedValidationPromise = this._Promise.resolve(validationPromise);
  
    this._validationOperations.add(wrappedValidationPromise);//兼容
  
    return validationPromise.then(isValid => {
      this._validationOperations.delete(wrappedValidationPromise); //兼容
    
      this._testOnBorrowResources.delete(pooledResource);

      if (isValid === false) {
        pooledResource.invalidate(); //标记为无效
        this._destroy(pooledResource); //关闭资源
        this._dispense(); //如果有等待的请求者,则重新分配资源
        return;
      }
      //将测试有效的资源分配给请求者
  ...
  _dispatchPooledResourceToNextWaitingClient(pooledResource) {
    ...
    pooledResource.allocate(); //标记为已借出
  ...
  clear() {
    ...
    return this._Promise.all(reflectedCreatePromises).then(() => {
      ...
      // @ts-ignore
      let resource = this._availableObjects.shift();
      while (resource) {
        this._destroy(resource, {ensureMinimum: false});
        resource = this._availableObjects.shift();
      }
      const reflectedDestroyPromises = Array.from(
        this._factoryDestroyOperations
      ).map(reflector);
      return reflector(this._Promise.all(reflectedDestroyPromises).then(v=> {
        this._ensureMinimum();
      }));
    });
  }
